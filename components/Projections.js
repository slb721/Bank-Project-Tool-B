...
export default function Projections({ refresh }) {
  ...
  useEffect(() => {
    async function run() {
      ...
      // 5) Build all cash-flow events over next 6 months
      const events = [];
      ...
      if (view === 'monthly') {
        datasets.push(
          ...
          },
          {
            ...
            data: dataFlow,
            yAxisID: 'y1',
            backgroundColor: dataFlow.map((f) =>
              f >= 0 ? '#10b981' : '#ef4444'
            ),
          },
          {
      ...
      const growth = (
        ((finalBal / +acct.current_balance - 1) * 2 * 100) ||
        0
      ).toFixed(2);
    ...
    <div className={styles.card}>
      ...
      <div className={styles.statsGrid}>
        <div className={styles.statCard}>
          ...
          {stats.negDay && (
            <p className={styles.statWarn}>
              ⚠️ Below zero on {stats.negDay}
            </p>
          )}
        </div>
        <div className={styles.statCard}>
